defprotocol Add do
  ...
  Fmap.apply([1,2,3,4,5], %Add{value: 10})
  ...
  """
  @enforce_keys [:value]
  defstruct [:value, fmap: &__MODULE__.fmap/2]
  def fmap(x, operation)
end
...
defimpl Add, for: Integer do
  def fmap(x, %{value: value}) when is_integer(value), do: Int.add(x, value)
  def fmap(x, %{value: value = %Decimal{}}), do: Decimal.to_integer(Decimal.round(value)) + x
end
...
defimpl Add, for: Decimal do
  def fmap(x, %{value: value = %Decimal{}}), do: Decimal.add(x, value)
  def fmap(x, %{value: value}) when is_integer(value), do: Decimal.add(x, Decimal.new(value))
end
...
defimpl Add, for: List do
  def fmap(list, operation), do: Fmap.apply(list, operation)
end
...
defimpl Add, for: Map do
  def fmap(map, operation), do: Fmap.apply(map, operation)
end
...
defimpl Add, for: PID do
  def fmap(pid, operation), do: Fmap.apply(pid, operation)
end
...
defimpl Add, for: Function do
  def fmap(stream, operation), do: Fmap.apply(stream, operation)
end
